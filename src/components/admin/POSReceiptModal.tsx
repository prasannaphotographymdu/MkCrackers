                ...
                {(order.items || []).map((it, idx) => (
                  <tr key={idx} className="text-[11px]">
                    <td className="py-1.5 pr-2 font-medium text-slate-800">
                      ...
                      <div className="text-[9px] text-slate-400 font-sans">SKU: {it.sku} &bull; GST {it.gstPercent}%</div>
                    </td>
                    ...
                    <td className="py-1.5 text-right font-bold text-slate-900">{formatINR(it.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="mt-3 pt-2 bg-slate-50 p-2.5 rounded-lg border border-slate-200 text-left font-sans text-[11px] space-y-1">
              ...
              {order.paymentMode === 'Cash' && (
                <div className="flex justify-between text-slate-600">
                  ...
                  <span className="font-bold text-slate-900">{formatINR(order.cashAmount || order.grandTotal)}</span>
                </div>
              )}
              {order.paymentMode === 'UPI' && (
                <div className="space-y-0.5">
                  <div className="flex justify-between text-slate-600">
                    ...
                    <span className="font-bold text-slate-900">{formatINR(order.upiAmount || order.grandTotal)}</span>
                  </div>
                  {order.upiRefNo && (
